...
pub fn get_echo(context:&Context, request: &HttpRequest) -> HttpResponse {
...
pub fn user_agent(context:&Context, request: &HttpRequest) -> HttpResponse {
...
pub fn get_file(context:&Context, request: &HttpRequest) -> HttpResponse {
    let parts: Vec<&str> = request.target.split('/').collect();
...
pub fn post_file(context:&Context, request: &HttpRequest) -> HttpResponse {
    let parts: Vec<&str> = request.target.split('/').collect();
    let file_name = parts[parts.len() - 1].to_string();
    let file_path = format!("{}/{}", request.files_directory.as_ref().expect("Files directory not present"), file_name);
...
pub fn html(context:&Context, request: &HttpRequest) -> HttpResponse {
    let parts: Vec<&str> = request.target.split('/').collect();
    let file_name = if parts[parts.len() - 1].to_string() == "" { "index.html".to_string() } else { parts[parts.len() - 1].to_string() };
    let file_path = format!("{}/{}", context.files_directory,file_name);
    let file_content = std::fs::read_to_string(file_path);
    match file_content {
        Ok(content) => HttpResponse::new(200, "OK".to_string(),
            vec![
                ("Content-Type".to_string(), "text/html\r\n".to_string()),
                ("Content-Length".to_string(), (content.len().to_string() + "\r\n").to_string()),
            ], content),
        Err(_) =>  HttpResponse::new(404, "Not Found".to_string(), vec![], "<h1>Pagina non trovata </h1>".to_string()),
    }
}
